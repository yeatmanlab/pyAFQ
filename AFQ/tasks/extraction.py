...
@mark.task
@mark.annotate(
    {"return": {"bundles_file": str}}
)
@as_file('_tractography.trk')
def segment(subses_tuple, streamlines_file, bundle_dict,
...
@mark.task
@mark.annotate(
    {"return": {"clean_bundles_file": str}}
)
@as_file('-clean_tractography.trk')
def clean_bundles(subses_tuple, bundles_file, bundle_dict, clean_params,
                  tracking_params={}, segmentation_params={}):
    img = nib.load(subses_tuple['dwi_file'])
    sft = load_tractogram(
        bundles_file,
...
@mark.task
def _export_bundles(subses_tuple, clean_bundles_file, bundles_file,
    ...
    for bundles_file, folder in zip([clean_bundles_file, bundles_file],
        ...
        bundles_dir = op.join(subses_tuple['results_dir'], folder)
...
@mark.task
@mark.annotate(
    {"return": {"sl_counts_file": str}}
)
@as_file('_sl_count.csv')
def _export_sl_counts(subses_tuple, bundle_dict,
