...
class AFQ(object):
    ...
    def __init__(self,
        ...
        self.custom_tractography_bids_filters=custom_tractography_bids_filters
        ...
        self.tracking_params["odf_model"]=self.tracking_params["odf_model"].upper(
        )
        ...
        if isinstance(
            ...
            self.segmentation_params["presegment_bundle_dict"]=make_bundle_dict(
    ...
    def _brain_mask(self, row):
        ...
        if not op.exists(brain_mask_file):
            brain_mask, brain_affine, meta=self.brain_mask_definition.get_for_row(
                self, row)
    ...
    def _export_seed_mask(self, row):
        if isinstance(self.tracking_params['seed_mask'], Definition):
            seed_mask, _, seed_mask_desc=self.tracking_params['seed_mask'].get_for_row(
                self, row)
        else:
    ...
    def _export_stop_mask(self, row):
        if isinstance(self.tracking_params['stop_mask'], Definition):
            stop_mask, _, stop_mask_desc=self.tracking_params['stop_mask'].get_for_row(
                self, row)
        else:
    ...
    def _streamlines(self, row):
        ...
        if not op.exists(streamlines_file):
            ...
            if isinstance(self.tracking_params['seed_mask'], Definition):
                tracking_params['seed_mask'], _, seed_mask_desc=self.tracking_params['seed_mask'].get_for_row(
                    self, row)
            else:
                seed_mask_desc=dict(source=tracking_params['seed_mask'])

            if isinstance(self.tracking_params['stop_mask'], Definition):
                tracking_params['stop_mask'], _, stop_mask_desc=self.tracking_params['stop_mask'].get_for_row(
                    self, row)
            else:
                stop_mask_desc=dict(source=tracking_params['stop_mask'])
        ...
        if not op.exists(bundles_file):
            ...
            for bundle in self.bundle_dict.keys():
                if bundle != "whole_brain":
                    row["sl_counts"].at[bundle, "n_streamlines"]=len(
                        bundles[bundle].streamlines)
            row["sl_counts"].at["whole_brain",
                "n_streamlines"]=len(tg.streamlines)
            row["sl_counts"].at["whole_brain",
                "n_clean_streamlines"]=len(tg.streamlines)
        return bundles_file
        ...
        if not op.exists(clean_bundles_file):
            ...
            for b in self.bundle_dict.keys():
                if b != "whole_brain":
                    ...
                    row["sl_counts"].at[b, "n_clean_streamlines"]=len(
                        this_tg.streamlines)
            ...
            if self.clean_params['return_idx']:
                afd.write_json(clean_bundles_file.split('.')[0] + '_idx.json',
                               return_idx)
            row['timing']['Cleaning']=row['timing']['Cleaning'] + time() - \
                                                                       start_time
    ...
    def _viz_bundles(self, row,
        ...
        row['timing']['Visualization']=row['timing']['Visualization'] + time() - \
                                                                             start_time
        return figure
    ...
    def set_dwi_img(self):
        self.data_frame['dwi_img']=self.data_frame['dwi_file'].apply(nib.load)
    ...
    def set_masked_b0(self):
        if 'masked_b0_file' not in self.data_frame.columns:
            self.data_frame['masked_b0_file']=self.data_frame.apply(self._b0_mask,
                                      axis=1)
    ...
    def set_brain_mask(self):
        if 'brain_mask_file' not in self.data_frame.columns:
            self.data_frame['brain_mask_file']=self.data_frame.apply(self._brain_mask,
                                      axis=1)
    ...
    def set_dti(self):
        if 'dti_params_file' not in self.data_frame.columns:
            self.data_frame['dti_params_file']=self.data_frame.apply(self._dti,
                                      axis=1)
    ...
    def set_dti_fa(self):
        if 'dti_fa_file' not in self.data_frame.columns:
            self.data_frame['dti_fa_file']=self.data_frame.apply(self._dti_fa,
                                      axis=1)
    ...
    def set_dti_cfa(self):
        if 'dti_cfa_file' not in self.data_frame.columns:
            self.data_frame['dti_cfa_file']=self.data_frame.apply(self._dti_cfa,
                                      axis=1)
    ...
    def set_dti_pdd(self):
        if 'dti_pdd_file' not in self.data_frame.columns:
            self.data_frame['dti_pdd_file']=self.data_frame.apply(self._dti_pdd,
                                      axis=1)
    ...
    def set_dti_md(self):
        if 'dti_md_file' not in self.data_frame.columns:
            self.data_frame['dti_md_file']=self.data_frame.apply(self._dti_md,
                                      axis=1)
    ...
    def set_dki(self):
        if 'dki_params_file' not in self.data_frame.columns:
            self.data_frame['dki_params_file']=self.data_frame.apply(self._dki,
                                      axis=1)
    ...
    def set_dki_mk(self):
        if 'dki_mk_file' not in self.data_frame.columns:
            self.data_frame['dki_mk_file']=self.data_frame.apply(self._dki_mk,
                                      axis=1)
    ...
    def set_dki_fa(self):
        if 'dki_fa_file' not in self.data_frame.columns:
            self.data_frame['dki_fa_file']=self.data_frame.apply(self._dki_fa,
                                      axis=1)
    ...
    def set_dki_md(self):
        if 'dki_md_file' not in self.data_frame.columns:
            self.data_frame['dki_md_file']=self.data_frame.apply(self._dki_md,
                                      axis=1)
    ...
    def set_dki_awf(self):
        if 'dki_awf_file' not in self.data_frame.columns:
            self.data_frame['dki_awf_file']=self.data_frame.apply(self._dki_awf,
                                      axis=1)
    ...
    def set_clean_bundles(self):
        if 'clean_bundles_file' not in self.data_frame.columns:
            if self.seg_algo == "reco":
                self.set_bundles()
                self.data_frame['clean_bundles_file']=self.data_frame['bundles_file']
            else:
    ...
    def set_template_xform(self):
        if 'template_xform_file' not in self.data_frame.columns:
            self.data_frame['template_xform_file']=self.data_frame.apply(self._template_xform,
                                      axis=1)
            ...
            if isinstance(self.bundle_info, list):
                ...
                for bundle in self._bundle_dict:
                    for ii, roi in enumerate(
                        ...
                        self._bundle_dict[bundle]['ROIs'][ii]=afd.read_resample_roi(
